...
const { showCategories, showOneCategory, createCategory, updateCategory, deleteCategory, } = require("../controllers/categoryController");
const { showPostsPerCategory } = require("../controllers/postController");
const { guard } = require("../middleware/authMiddleware");
router.route("/").get(showCategories).post(guard, createCategory);
router
    .route("/:category")
